...
int16_t pwm_periodA;  // Holds the pwm commands
uint16_t hip_enc1, knee_enc1, heel_enc1, spring_enc1;
double hip_enc2, knee_enc2, heel_enc2, spring_enc2;
...
//void positionSensorCallback1(const nodes::LegEncoders::encoders_msg& value)
void positionSensorCallback1(const std_msgs::Float64::ConstPtr& value)
{
  
	//ROS_INFO("position sensor sent value in rad  %f", value->data);
	...
	//
	hip_enc1 = (int32_t)( 102000 * ( ( value->data * RAD2DEG ) / 360 ) );
	//
  	knee_enc1 = 0; 
  	heel_enc1 = 0;
  	spring_enc1 = 0;
	//
        // ROS_INFO("position sensor sent value counts %d \n", encoders_msg.encoder_hip);
...
//void positionSensorCallback2(const nodes::LegEncoders::ConstPtr& value)
...
int main(int argc, char **argv)
{
  ...
  set_torque_srv1.request.torque =  MAXTORQUE * pwm_periodA * 0.001;
  ROS_INFO("torque1 set to: %f", set_torque_srv1.request.torque );
...
*/
  ...
  if (time_step_client.call(time_step_srv) && time_step_srv.response.success == 2)
	...
	while (ros::ok())
    {
		...
		encoders_msg.encoder_hip = hip_enc1;
		encoders_msg.encoder_knee = knee_enc1;
		encoders_msg.encoder_heel = heel_enc1;
		encoders_msg.encoder_spring = spring_enc1;
		...
		/*
		if (i5<100){
			set_torque_srv1.request.torque =  5; //MAXTORQUE * pwm_periodA*0.001;
			set_torque_client1.call(set_torque_srv1);
			//ROS_INFO("torque1 set to: %f", set_torque_srv1.request.torque );
			ROS_INFO("step No: %d", i5 );
		}else{
			set_torque_srv1.request.torque =  -5; //MAXTORQUE * pwm_periodA*0.001;
			set_torque_client1.call(set_torque_srv1);
			//ROS_INFO("torque1 set to: %f", set_torque_srv1.request.torque );
		...
		*/
		...
		//ROS_INFO("torque1 set to: %f", set_torque_srv1.request.torque );
        	if (i5%50==0){
			ROS_INFO("step No::: %d", i5 );
			ROS_INFO("PWM value: %d", pwm_periodA );
			ROS_INFO("Torque value: %f", MAXTORQUE * pwm_periodA*0.01 );
			ROS_INFO("hip_enc1: %d", hip_enc1 );
			ROS_INFO("hip_enc2: %f", hip_enc2 );
		}		
		
                // **************************************************************************************************
		//time_step_client.call(time_step_srv);
		//ros::spinOnce();
		//ROS_INFO_STREAM(encoders_msg);
		set_torque_srv1.request.torque =  MAXTORQUE * pwm_periodA * 0.01;
		ros::spinOnce();
		set_torque_client1.call(set_torque_srv1);
		loop_rate.sleep();
    }
